...
| Diff non commité dans l'arbre de travail | session interrompue avant son commit, ou vague verrouillée en cours (`WORKFLOW.md` §4b) | `/fin-de-tache` de la session interrompue (repère : le `S<k>.md` dont la zone contient les fichiers) ; la fin de plan seulement une fois toutes les sessions commitées |
...
3. La session suivante prête dans l'`index.md` → le **bloc de relance** (`/fin-de-tache`, domicile),
   pastille en plus en Desktop — **jamais l'exécuter dans cette conversation** : une session part
   toujours à froid (`WORKFLOW.md` §5b).
